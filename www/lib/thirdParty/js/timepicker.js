    ...
    bindEvent: function(type) {
      ...
      function popupDateTime(e) {
        ...
        _self.gearDate.innerHTML = '<div class="date_ctrl slideInUp">' +
          ...
          '<p id="alertWarn">选择时间超出车位时间</p>' +
          '<div class="date_btn lcalendar_finish" id="lendDate">确定</div>' +
          '</div>' +
      ...
      function dateTimeCtrlInit() {
        var date;
        if (_self.state==='start') {
          console.log(_self.start);
          date = new Date(_self.start);
        } else if (_self.state==='end') {
          date = new Date(_self.end);
        } else {
          date = new Date();
        };        
        var dateArr = {
      ...
      function closeMobileCalendar(e) {
        ...
        document.getElementsByClassName('date_ctrl')[0].classList.toggle('slideDown');
        document.getElementsByClassName('date_ctrl')[0].classList.toggle('date_ctrlno');
        setTimeout(function() {
      ...
      function finishMobileDateTime(e) {
        ...
        var lastValue = (date_yy % passY + _self.minY) + "-" + date_mm + "-" + date_dd + " " + (time_hh.length < 2 ? "0" : "") + time_hh + (time_mm.length < 2 ? ":0" : ":") + time_mm;
        
        if (_self.state==='start') {
          if(Date.parse(lastValue) >= Date.parse(_self.start)&&Date.parse(lastValue)<Date.parse(_self.end)){
            _self.trigger.value = lastValue;
            closeMobileCalendar(e);
          }else{
            document.getElementById('alertWarn').style.display = 'block';
          }
        } else if (_self.state==='end') {
          if(Date.parse(lastValue) <= Date.parse(_self.end)&&Date.parse(lastValue)>Date.parse(_self.start)){
            _self.trigger.value = lastValue;
            closeMobileCalendar(e);
          }else{
            document.getElementById('alertWarn').style.display = 'block';
